...
$PythonScript = Join-Path $Root 'scripts\prototypes\reazon_k2_transcribe.py'

$Timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$RunDir = Join-Path $Root "runs\${Timestamp}_reazon_k2_int8"
...
try {
    ...
    Assert-FileExists -Path $PythonScript -Label 'ReazonSpeech Python script'
    Assert-DirectoryExists -Path (Join-Path $Root '.venv') -Label '.venv'

    $totalStart = Get-Date
    Write-Log "START run_reazon_k2_test"
    ...
    Write-Log ('convert_10min_seconds={0:N3}' -f $convertSeconds)
    ...
    $MonitorJob = Start-Job -ArgumentList $ResourceLog -ScriptBlock {
        ...
        while ($true) {
            ...
            '{0},{1},{2},{3},{4},{5},{6},{7}' -f `
                (Get-Date -Format 'o'), $totalMb, $availableMb, $usedMb, $pythonMb, $pythonCpu, $ffmpegMb, $ffmpegCpu |
    ...
    Write-Log "END run_reazon_k2_test OK"
