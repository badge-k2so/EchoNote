param(
    ...
    [string]$ModelDir = "models\sherpa-onnx-nemo-parakeet-tdt-0.6b-v2-int8",
...
$AsrScript  = Join-Path $Root 'scripts\prototypes\sherpa_onnx_transducer_transcribe.py'
...
try {
    ...
    Assert-FileExists -Path $Encoder    -Label 'Parakeet encoder'
    Assert-FileExists -Path $Decoder    -Label 'Parakeet decoder'
    Assert-FileExists -Path $Joiner     -Label 'Parakeet joiner'
    Assert-FileExists -Path $Tokens     -Label 'Parakeet tokens'

    $totalStart = Get-Date
    Write-Log "START run_sherpa_onnx_parakeet_test"
    Write-Log "InputFile=$InputFile"
    Write-Log "ModelDir=$ModelDir NumThreads=$NumThreads"
    ...
    Invoke-LoggedProcess -Exe $VenvPython -Arguments $pyArgs `
        -OutputLog (Join-Path $LogsDir 'sherpa_onnx_stdout.txt') -FailureMessage 'sherpa-onnx Parakeet failed.'
    ...
    Write-Log "END run_sherpa_onnx_parakeet_test OK"
