"""sherpa-onnx transducer offline transcription for VAD chunks."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="sherpa-onnx transducer CPU transcription")
    ...
    parser.add_argument("--encoder", required=True, help="Transducer encoder ONNX path")
    parser.add_argument("--decoder", required=True, help="Transducer decoder ONNX path")
    parser.add_argument("--joiner", required=True, help="Transducer joiner ONNX path")
    parser.add_argument("--tokens", required=True, help="Tokens path")
    parser.add_argument("--model_type", default="nemo_transducer")
    parser.add_argument("--num_threads", type=int, default=2)
    parser.add_argument("--provider", default="cpu")
    parser.add_argument("--decoding_method", default="greedy_search")
    parser.add_argument("--max_active_paths", type=int, default=4)
    parser.add_argument("--edge_silence_ms", type=float, default=0.0)
    parser.add_argument("--edge_fade_ms", type=float, default=10.0)
    parser.add_argument("--normalize_peak", type=float, default=0.0)
...
def prepare_audio(
    ...
    if edge_silence_ms > 0 and len(samples):
        fade_samples = min(
            len(samples),
            max(0, round(sample_rate * edge_fade_ms / 1000)),
        )
        if fade_samples:
        ...
        silence_samples = max(0, round(sample_rate * edge_silence_ms / 1000))
        samples = np.pad(samples, (silence_samples, silence_samples))

    if normalize_peak > 0 and len(samples):
...
def main() -> int:
    ...
    append_log(log_path, "sherpa_onnx_transducer_start")
    ...
    append_log(log_path, f"joiner={args.joiner}")
    append_log(log_path, f"tokens={args.tokens}")
    append_log(log_path, f"model_type={args.model_type} num_threads={args.num_threads} provider={args.provider}")
    append_log(
        log_path,
        "input_preparation="
        f"edge_silence_ms={args.edge_silence_ms} "
        f"edge_fade_ms={args.edge_fade_ms} "
        f"normalize_peak={args.normalize_peak} "
        f"max_active_paths={args.max_active_paths}",
    )
    ...
    for index, chunk_path in enumerate(chunks):
        ...
        try:
            ...
            samples = prepare_audio(
                samples,
                sample_rate,
                args.edge_silence_ms,
                args.edge_fade_ms,
                args.normalize_peak,
            )
